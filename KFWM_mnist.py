...
parser.add_argument('--dataset', type=str, default='mnist', choices=['mnist', 'cifar100', 'svhn', 'cifar10', 'caltech101', 'nyuv2'],
...
def gen_I(scheme,N):
    ...
    us=torch.rand(N,100,1,1).to(device)
    if scheme==1:
        Is=10*torch.rand(N,1,32,32).to(device)
        Is=Is.detach()
    ...
    if scheme==3:
        ...
        Is.requires_grad=False
    if scheme==4:
        Is=torch.rand(N,1,32,32).to(device)
        Is=Is.detach()
        Is.requires_grad=False
        for n in range(N):
...
def DFD(model,generator,E,t_loader,t,verbose):
    ...
    for epoch in range(0,E):
        ...
        z=torch.rand((15*N,100,1,1)).to(device)
        ...
        if epoch%(int(E/10))==0 and verbose:
            mnist_acc=test(args,local_model,device,test_loader,epoch)
            trigger_acc=test(args,local_model,device,t_loader,epoch)
...
def ItoT(I,Q,EE,EEE,lam,verbose=False):
    ...
    for q in range(Q):
        ...
        for epoch in range(EE):
            z=torch.rand((args.batch_size,100,1,1)).to(device)
            fake=generator(z).detach()
            y=model(fake).detach()
            ty=model_q(fake)
            loss=F.l1_loss(y,ty)
...
# Model loaded.
generator=network.gan.GeneratorA(nz=100,nc=1,img_size=32)
generator.load_state_dict(torch.load("./checkpoint/student/mnist-lenet5_half-generator.pt"))
generator.eval()
generator=generator.to(device)
# Generator loaded.
# For scheme2.
generator2=network.gan.GeneratorA(nz=100,nc=1,img_size=32)
generator2.eval()
...
if test_branch==3:
    #1,2 2000
    #3 300
    Mwm=DFD(model,generator,5000,I_loader,I,True)
if test_branch==4:
    T,T_loader=ItoT(I,30,20,20,0.05,True)
    DFD(model,generator,1000,T_loader,T,True)
if test_branch==5:
