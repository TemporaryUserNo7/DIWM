...
parser.add_argument('--dataset', type=str, default='caltech101', choices=['mnist', 'cifar100', 'svhn', 'cifar10', 'caltech101', 'nyuv2'],
                        help='dataset name (default: mnist)')
parser.add_argument('--model', type=str, default='resnet34', choices=['lenet5', 'resnet34', 'resnet34_8x'],
...
def gen_I(scheme,N):
    ls=np.random.randint(0,100,size=N)
...
def DFD(model,generator,E,t_loader,t,verbose):
    ...
    local_model=local_model.to(device)
    #0.00002
    local_optimizer=optim.Adam(local_model.parameters(),lr=0.00001)
    hm=[]
    ht=[]
    for epoch in range(0,E):
        local_optimizer.zero_grad()
        #z=torch.rand((10*N,100,1,1)).to(device)
        z=torch.rand((10*N,256,1,1)).to(device)
        fake=generator(z).detach()
        #generator is global
        y=model(fake).detach()
        #model is global
        ty=local_model(fake)
        loss=10*F.l1_loss(y,ty)
        ...
        if epoch%(int(E/5))==0 and verbose:
            mnist_acc=test(args,local_model,device,test_loader,epoch)
            trigger_acc=test(args,local_model,device,t_loader,epoch)
...
print(args)
train_dataset=ct101.ct101_build("train")
test_dataset=ct101.ct101_build("test")
...
# Model loaded.
generator=network.gan.GeneratorB(nz=256,nc=3,img_size=128)
generator.load_state_dict(torch.load("./checkpoint/student/caltech101-resnet18-generator34.pt"))
generator.eval()
generator=generator.to(device)
# Generator loaded.
# For scheme2.
generator2=network.gan.GeneratorB(nz=256,nc=3,img_size=128)
generator2.eval()
...
if test_branch==3:
    #1,2 2000
    #3 300
    DFD(model,2000,I_loader,I,True)
if test_branch==4:
    T,T_loader=ItoT(I,30,20,20,0.05,True)
    DFD(model,1000,T_loader,T,True)
if test_branch==5:
